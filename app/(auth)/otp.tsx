...
interface OTPTheme {
   pinCodeContainerStyle: object;
   pinCodeTextStyle: object;
   focusStickStyle: object;
   focusedPinCodeContainerStyle: object;
   placeholderTextStyle: object;
   filledPinCodeContainerStyle: object;
   disabledPinCodeContainerStyle: object;
}
...
const OTPVerificationScreen: React.FC = () => {
   ...
   const handleResendOtp = async (): Promise<void> => {
      if (timeLeft > 0 || isResending || verificationStatus === 'success')
         return;
   ...
   const handleVerify = async (): Promise<void> => {
      ...
      try {
         ...
         if (otp === '123456') {
            ...
            setTimeout(() => {
               router.replace('/(protected)'); // Replace with your route
            }, 1000);
         } else {
            ...
            <VStack className="flex-1 px-6" space="xl">
               ...
               <Center className="justify-center my-8">
                  <Animated.Image
                     ...
                     resizeMode="contain"
                     accessibilityLabel="Logo de l'application Delivery"
                  />
                  <Animated.Text
               ...
               <HStack className="justify-center space-x-2 mb-4">
                  {[1, 2, 3].map(step => (
                     <View
                        key={`step-${step}`}
                        className={`h-1 rounded-full ${step === 2 ? 'bg-primary-500 w-16' : 'bg-gray-200 w-8'}`}
                     />
                  ))}
               </HStack>
               ...
               <View>
                  ...
                  <Text className="text-center text-gray-600 mb-6">
                     Veuillez entrer le code à 6 chiffres envoyé sur votre
                     numéro de téléphone pour vérifier votre compte.
                  </Text>
               </View>
               ...
               {verificationStatus && (
                  ...
                  >
                     <HStack className="items-center my-2" space="xs">
                        ...
                        />
                        <Text
                           className={`${verificationStatus === 'success' ? 'text-green-600' : 'text-red-500'}`}
                        >
                           {verificationStatus === 'success'
               ...
               <Pressable
                  ...
                  className={`items-center py-2 ${timeLeft === 0 ? 'opacity-100' : 'opacity-70'}`}
               >
                  {isResending ? (
                     ...
                     <Text
                        size="lg"
                        className={`text-center font-bold ${timeLeft === 0 ? 'text-primary-500' : 'text-gray-500'}`}
                     >
                        {timeLeft === 0
                           ? 'Renvoyer le code'
               ...
               <Animated.View style={[{ width: '100%' }, buttonAnimatedStyle]}>
                  <Button
                     size="xl"
                     className={`my-4 w-full ${!isConfirmEnabled ? 'opacity-70' : ''}`}
